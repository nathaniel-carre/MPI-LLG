...
let cardinal couv =
    (* Calcule le cardinal d'un ensemble représenté par tableau de booléens. *)
    Array.fold_left (fun a b -> a + if b then 1 else 0) 0 couv

let couverture_min g =
    (* Calcule la couverture minimale. On procède par backtracking. Chaque fois
       qu'on tombe sur une solution totale, on vérifie si c'est une couverture, 
       et on compare à la meilleure couverture trouvée, en commençant par le cardinal,
       puis par l'ordre lexicographique des sommets. Ici, on se sert de la comparaison
       naturelle des tableaux. *)
    let n = Array.length g in
    let couv = Array.make n false in
    let couv_min = ref [||] and cardmin = ref (n + 1) in
    let rec backtracking i = 
        if i = n then begin
            if est_couverture g couv then begin
                let card = cardinal couv in
                if (-card, couv) > (- !cardmin, !couv_min) then
                    (couv_min := Array.copy couv; cardmin := card)
            end
        end else begin
            backtracking (i + 1); couv.(i) <- true;
            backtracking (i + 1); couv.(i) <- false
        end
    in
    backtracking 0;
    !couv_min
