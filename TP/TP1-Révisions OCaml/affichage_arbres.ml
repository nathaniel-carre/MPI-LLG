...
let rec affichage_bin_aux a = 
    ...
    let sx = racine_bin a and g, d = fils_bin a in
    let nx = String.length sx in
    match g, d with
       | Vide, Vide -> [sx], nx, nx / 2, nx mod 2
       | _, Vide    -> let lstg, larg, posg, parg = affichage_bin_aux g in
       ...
       | Vide, _    -> let lstd, lard, posd, pard = affichage_bin_aux d in
       ...
       | _          -> let lstg, larg, posg, parg = affichage_bin_aux g and
                           lstd, lard, posd, pard = affichage_bin_aux d in
              ...
              let rec fusion lg ld = match lg, ld with
                  ...
                  | sg :: qg, sd :: qd -> let ng = String.length sg in
                           (sg ^ String.make (- ng + nouv_posd - posd) ' ' ^ sd) :: fusion qg qd in
...
let liste_fils (Noeud(_, lst)) = lst;;
...
let rec affichage_aux a =
    (* Prend en argument un arbre non vide et renvoie un tuple 
       formé de :
        - la liste des lignes à afficher ;
        - la largeur totale à afficher. *)
    let sx = racine a and lst_fils = liste_fils a in
    let nx = String.length sx in
    let lst_aff = List.map affichage_aux lst_fils in
    match lst_aff with
        | []           -> [sx], nx
        ...
        let rec lgn2 = function
            ...
            | (_, lar) :: q -> String.make lar '-' ^ "+" ^ lgn2 q in
        ...
        let rec fusion l1 l2 decal = match l1, l2 with
            ...
            | s1 :: q1, s2 :: q2 -> let n1 = String.length s1 in
               (s1 ^ String.make (max 0 (decal - n1)) ' ' ^ s2) :: fusion q1 q2 decal in
        let rec fusion_lst = function
            ...
            | [(lst, lar)]                  -> [(lst, lar)]
            | (l1, lar1) :: (l2, lar2) :: q -> 
                    fusion_lst ((fusion l1 l2 (lar1 + 1), lar1 + lar2 + 1) :: q) in
