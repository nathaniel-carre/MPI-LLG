(* Le chemin suivant est à remplacer par le chemin du dossier où vous 
   ...
   Vous pouvez ne rien mettre si tous les fichiers sont dans le dossier courant. *)
let chemin = ""

type image = int array array
type donnees = image array * int array
...
let ouvrir set =
    ...
    let imgs = Array.make nb [||] and
        etqs = Array.make nb (-1) in
    for i = 0 to nb - 1 do
        imgs.(i) <- Array.make_matrix hauteur largeur 0;
        etqs.(i) <- input_byte etiquettes;
        for lgn = 0 to hauteur - 1 do
            for col = 0 to largeur - 1 do
                imgs.(i).(lgn).(col) <- input_byte images
            done
        done
   done;
   imgs, etqs

let train = ouvrir "train" and test = ouvrir "t10k"
...
let afficher im pixel =
    ...
    for lgn = 0 to h - 1 do
        for col = 0 to l - 1 do
            ...
            Graphics.fill_rect (pixel * col) (pixel * (h - lgn - 1)) pixel pixel;
        done
    done;;
*)
