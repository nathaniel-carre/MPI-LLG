...
// Pas de difficulté pour les fonctions d'incrémentation. On pense à faire le verrouillage
// à l'endroit de l'incrémentation, pas en dehors des boucles.
...
// À ne pas confondre avec le tri par sélection ! La boucle while permet d'avoir
// une complexité linéaire dans le meilleur cas.
...
// L'instruction &tr.debut[i+1] ne déclenche pas d'erreur, même si i est le dernier
// indice du tableau.
void* tri_rapide1(void* arg){
    tranche tr = *(tranche*) arg;
    if (tr.taille > SEUIL_TRI){
        int i = partition(tr);
        tranche tr0 = {.debut = tr.debut, .taille = i};
        tranche tr1 = {.debut = &tr.debut[i+1], .taille = tr.taille - i - 1};
        tri_rapide1(&tr0);
        tri_rapide1(&tr1);
    } else {
        tri_insertion(&tr);
...
// Lorsqu'on doit faire deux appels récursifs, on en enfile un, et on traite l'autre. On
// pense à mettre à jour taille_max (pas besoin de verrouiller, car c'est un atomic_int).
...
// Un fil d'exécution essaie de traiter des tranches tant qu'il y en a à défiler.
// L'argument taille_max permet de savoir s'il reste du travail (car la file pourrait
// être temporairement vide).
...
// On crée autant de fils que nécessaire, qui vont essayer de traiter des tâches.
