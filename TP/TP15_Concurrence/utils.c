...
// Pour les deux fonctions suivantes, on suit l'algorithme du cours.

void verrouiller(mutex* m, int i){
    m->attente[i] = true;
    m->tour = 1 - i;
    while (m->tour != i && m->attente[1 - i]){}
}
...
// Pas de difficulté particulière, il faut bien tout initialiser.
...
// On parcourt chaque maillon jusqu'à arriver au dernier. On pense également à détruire le mutex.
...
// On ne touche pas au champ taille_max ici. On se contente de créer un nouveau maillon,
// puis on l'ajoute au reste de la file en penser à verrouiller le mutex.
...
// On commence par verrouiller le mutex avant de faire des modifications. Ensuite,
// on défile le premier maillon, en pensant à traiter à part le cas où c'était le dernier
// maillon.
