...
let union_matrice m1 m2 =
    ...
    Array.init n (fun i -> union m1.(i) m2.(i));;

(* À nouveau, la concaténation doit bien être traitée, car F(ef) = F(e) ∪ F(f) ∪ S(e)P(f). 
   Pour l'étoile, on constate que F( e * ) = F(ee), ce qui évite de tout réécrire. *)
