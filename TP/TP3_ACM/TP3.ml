...
let _ =
    let aux i g = 
        Printf.printf "Le poids de l'arbre couvrant minimal du graphe d'indice %d est %d\n" 
                      i (poids_graphe (kruskal g)) 
    in
    List.iteri aux graphes

(* Exercice déjà corrigé en classe. La référence num garde en mémoire 
   le numéro de la composante courante. On fait un parcours en profondeur 
   en attribuant le même numéro à tous les sommets atteints, puis on 
   incrémente la référence, avant de passer au sommet suivant. On ignore
   complètement les poids pour cette fonction. *)
let composantes h =
...
(* On crée le tableau, avec une valeur inutile. Ensuite, on parcourt toutes
   les arêtes de G, et si une arête relie deux sommets de CC différentes, on
   l'envisage comme arête sûre pour chacune des deux composantes. *)
let aretes_sures g h cc m =
    let n = Array.length g in
    let sures = Array.make m (max_int, n, n) in
    for s = 0 to n - 1 do
        let traiter (t, p) =
            if cc.(s) <> cc.(t) then begin 
                let a = (p, min s t, max s t) in
                if a < sures.(cc.(s)) then sures.(cc.(s)) <- a;
                if a < sures.(cc.(t)) then sures.(cc.(t)) <- a
            end
        in
...
(* On parcourt les arêtes sûres pour les ajouter. La difficulté est d'éviter
   les doublons. Pour cela, on ajoute une arête pour la composante du premier
   sommet systématiquement, et pour le deuxième sommet seulement si on ne l'a
   pas déjà fait pour le premier sommet. *)
let ajouter_aretes_sures h cc sures =
    let m = Array.length sures in
    for j = 0 to m - 1 do
        let (p, s, t) = sures.(j) in
        if cc.(s) = j || sures.(cc.(s)) <> sures.(j) then begin
            h.(s) <- (t, p) :: h.(s);
            h.(t) <- (s, p) :: h.(t)
        end
    done

(* On applique l'algorithme tel qu'il est décrit. *)
let boruvka g =
    let n = Array.length g in
    let h = Array.make n [] in
    let m = ref n in
    while !m > 1 do
        let cc, m' = composantes h in
        m := m';
        if !m > 1 then
            let sures = aretes_sures g h cc !m in
            ajouter_aretes_sures h cc sures
    done;
    h

(* On calcule les poids des ACM, pour vérifier la cohérence avec Kruskal. *)
let _ =
    let aux i g = 
        Printf.printf "Le poids de l'arbre couvrant minimal du graphe d'indice %d est %d\n" 
                      i (poids_graphe (boruvka g)) 
    in
    List.iteri aux graphes

let _ =
    let aux i g =
        Printf.printf "Pour le graphe d'indice %d : \n" i;
        let t0 = Sys.time () in
        ignore (kruskal g);
        let t1 = Sys.time () in
        ignore (boruvka g);
        let t2 = Sys.time () in
        Printf.printf "Le temps de calcul est de %f pour Kruskal et %f pour Boruvka.\n"
            (t1 -. t0) (t2 -. t1)
    in
    List.iteri aux graphes