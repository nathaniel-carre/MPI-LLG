...
let infix tree =
    let rec infix_aux tree acc = match tree with
        | Nil -> acc
        | Node(l, i, r) -> infix_aux l (i :: infix_aux r acc) 
    in
    infix_aux tree []
...
let height_explored n k =
    let data = create_data n k in
    let d1, t1 = kd_tree data and
        d2, t2 = kd_tree_median data in
    let x = create_point k in
    height t1, snd (nearest_neighbor d1 t1 x),
    height t2, snd (nearest_neighbor d2 t2 x)

let test2 n k p =
    let sh1 = ref 0 and sh2 = ref 0 and
	    se1 = ref 0 and se2 = ref 0 in
	for _ = 0 to p - 1 do
		let h1, e1, h2, e2 = height_explored n k in
        sh1 := !sh1 + h1;
		sh2 := !sh2 + h2;
		se1 := !se1 + e1;
		se2 := !se2 + e2
    ...
    let fp = float p in
	float !sh1 /. fp, float !se1 /. fp, 
    float !sh2 /. fp, float !se2 /. fp

let _ = 
    let f k =
        let sh1, se1, sh2, se2 = test2 100_000 k 10 in
        Printf.printf "Pour k = %d :
        - il y a en moyenne %f nœuds explorés dans le premier arbre, de hauteur %f.
        - il y a en moyenne %f nœuds explorés dans le deuxième arbre, de hauteur %f.\n" 
            k se1 sh1 se2 sh2; 
    in
    List.iter f [3; 10; 20]

(* L'arbre équilibré est toujours de hauteur 16. L'arbre aléatoire a une hauteur entre
   38 et 40, soit un peu plus du double. En termes de nombre de nœuds explorés, il n'y
   a toutefois pas de différence significative. *)
